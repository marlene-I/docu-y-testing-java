...
import mutation.Actividad;
import mutation.CupoExcedidoException;
import mutation.EdadInsuficienteException;
import mutation.NominaSocios;
import mutation.Persona;
import mutation.Socio;
import mutation.YaExisteSocioException;
    ...
    public void testInscripcionSocio(Socio socio) throws CupoExcedidoException, EdadInsuficienteException {
        ...
        if (this.actividad.getInscriptos().size() < this.actividad.getCupo()) {
            if (socio.getPersona().getEdad() < this.actividad.getEdadMinima()) {
                Assert.assertThrows(EdadInsuficienteException.class, () -> {
            ...
            Assert.assertThrows(CupoExcedidoException.class, () -> {
                this.actividad.inscribirSocio(socio);
            });
        }
